...
class DataCleaner:
    # please input the formatted data only. e.g. before_clean, after_clean
    def __init__(self, data: pd.DataFrame):
        self.data = data

    def generate_filter_reason(self, county_data, coefficient) -> pd.DataFrame:
        mean_series = county_data.mean()
        variance_series = county_data.std()
        ...
        # Add more columns to be filtered here if needed.
        for i in columns:
            county_data = self.__filter_outlier(county_data, i, mean_series, variance_series, coefficient)
        return county_data

    def __filter_outlier(self, county_data, column_name, mean_series, variance_series, coefficient) -> pd.DataFrame:
        mean = mean_series[column_name]
        variance = variance_series[column_name]
        condition = (county_data[column_name] != np.nan) \
                    & ((county_data[column_name] > (mean + coefficient * variance))
                       | (county_data[column_name] < (mean - coefficient * variance)))

        county_data.loc[condition, 'filter_reason'] = county_data['filter_reason'] + "{0} out of bound;".format(
            column_name)

        if column_name == "Population":
            if len(county_data[county_data['filter_reason'] != ""]) > 0:
                county_data['keep'] = False
        else:
            county_data.loc[condition, column_name] = np.nan

        return county_data

    def clean_data(self, coefficient: float):
        county_name_list = self.data["Name"].unique().tolist()
        result_list = []
        throw_away_list = []

        for index, county_name in enumerate(county_name_list):
            # print(index)
            # if index>100:
            #     break
            county_data = self.data[self.data["Name"] == county_name]
            if not county_data.empty:
                county_data.loc[:, "filter_reason"] = ""
                county_data.loc[:, "keep"] = True
                county_data_with_filter_reason = self.generate_filter_reason(county_data, coefficient)
                if county_data_with_filter_reason.iloc[0]["keep"] == False:
                    throw_away_list.append(county_data_with_filter_reason)
                else:
                    result_list.append(county_data_with_filter_reason)
        result = pd.concat(result_list) if len(result_list) > 0 else pd.DataFrame()
        throw_away = pd.concat(throw_away_list) if len(throw_away_list) > 0 else pd.DataFrame()
        return result, throw_away


if __name__ == '__main__':
    data = pd.read_csv("before_clean.csv")
    coefficient = 1.8

    data_cleaner = DataCleaner(data)
    data_after_clean, data_thrown_away = data_cleaner.clean_data(coefficient)
    data_after_clean.to_csv("CountyData_after_clean.csv")
    data_thrown_away.to_csv("CountyData_throw_away.csv")
